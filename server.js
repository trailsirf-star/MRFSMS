...
async function buyNumberFromProviderWithRetry(countryId, provider, serviceCode = 'wa', maxDurationMs = 30000, delayMs = 3000) {
    let lastError = 'No number from provider';
    const deadline = Date.now() + maxDurationMs;
    while (Date.now() < deadline) {
        const remainingMs = deadline - Date.now();
        const result = await buyNumberFromProvider(countryId, provider, serviceCode, Math.max(1, Math.min(15000, remainingMs)));
...
async function buyNumberFromProvider(countryId, provider, serviceCode = 'wa', timeoutMs = 15000) {
