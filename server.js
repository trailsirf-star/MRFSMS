...
async function buyNumberFromProvider(countryId, provider, serviceCode = 'wa', websitePricePkr = null, timeoutMs = 15000) {
    ...
    if (!Number.isFinite(providerId) || providerId <= 0) {
        return makeStrictPriceBlockedResponse({ reason: 'provider_id_unavailable' });
    }
    let liveProviders = [];
    try {
        liveProviders = await fetchProviderTiers(countryId, serviceCode);
    } catch (err) {
        return makeStrictPriceBlockedResponse({ reason: 'live_provider_price_fetch_failed', providerError: err.message });
    }
    const liveProvider = liveProviders.find((candidate) => Number(candidate.provider_id) === providerId) || null;
    if (!liveProvider) {
        return makeStrictPriceBlockedResponse({ reason: 'live_provider_price_unavailable', providerId });
    }
    const providerPrice = Number(liveProvider.price);
    if (!Number.isFinite(providerPrice) || providerPrice < 0) {
        return makeStrictPriceBlockedResponse({ reason: 'provider_price_unavailable', providerId });
    }
    const strictProvider = { ...provider, provider_id: providerId, price: providerPrice };
    const preflight = validateProviderPriceBeforePurchase(websitePricePkr, strictProvider);
    if (!preflight.allowed) return makeStrictPriceBlockedResponse(preflight);
    const strictMaxPrice = pkrToUsd(preflight.websitePricePkr);
    const url =
    ...
    try {
        ...
        if (parsed.success) {
            ...
            const postflight = validateProviderPriceAfterPurchase(preflight.websitePricePkr, guardedResult);
            if (!postflight.allowed) return makeStrictPriceBlockedResponse({ ...postflight, activationId: guardedResult.activationId });
...
async function buyNumberByTierStrategy(countryId, clientMaxUsd, serviceCode = 'wa', websitePricePkr = null) {
    try {
        ...
        const affordableProviders = providers
            .filter((p) => validateProviderPriceBeforePurchase(serverWebsitePrice, p).allowed)
            .slice(0, 5);
